...
$(document).ready(function() {
	...
	$(".cancel").click(function() {
		...
		<script type="text/javascript">
function toAdd() {
	window.location.href = "<%=basePath%>student/toAdd.do";
}
function toList() {
	window.location.href = "<%=basePath%>student/toList.do";
}
	...
	<%
		...
		<div class="place">
			...
			<ul class="placeul">
				...
				<li>
					<a href="<%=basePath%>goods/toList.do">学生信息管理</a>
				</li>
				<li>
					<a href="<%=basePath%>goods/toList.do">学生列表</a>
				</li>
			</ul>
		</div>
		<div class="rightinfo">
			...
			<div class="tools">
				...
				<form id="deleteAlls" action="<%=basePath%>student/toDelete.do"
					method="post">
					<input type="hidden" id="deleteB" name="studentId" />
				</form>
				<form id="searchAll" action="<%=basePath%>student/toList.do"
					method="post">
			...
			<tbody>
				...
				<c:if test="${!empty studentList}">
					<c:forEach items="${studentList}" var="st">
						<tr>
							<td><input name="select" type="checkbox" value="${st.studentId}" />
							</td>
							<td><%=count%></td>
							<td>${st.name}</td>
							<td>${st.sex}</td>
							...
							<td>${st.nation}</td>
							<td>${st.birthDate}</td>
							<td>${st.entranceDate}</td>
							<td>${st.politics}</td>
							<td>${st.major}</td>
							<td>
								<form id="myFormDelete<%=count%>" action="<%=basePath%>student/toDelete.do"
									method="post">
									<input type="hidden" name="studentId" value="${st.studentId}" />
								</form>
								<form id="myFormUpdate<%=count%>" action="<%=basePath%>student/toUpdate.do"
									method="post">
									<input type="hidden" name="studentId" value="${st.studentId}" />
								</form> 
								<a href="javascript:void(0)" onclick="update(<%=count%>)">修改</a>&nbsp;&nbsp; 
								<a href="javascript:void(0)" onclick="deleteStudent(<%=count++%>)">删除</a>&nbsp;
							</td>
						</tr>
					</c:forEach>
				</c:if>
